...
public class SearchSpecification<M extends Model<T>, T extends Serializable> implements Specification<M> {
    ...
    private Predicate predicate(Condition condition, Root<M> root, CriteriaBuilder criteriaBuilder) {
        try {
            switch (condition.getComparison()) {
                ...
                case RELATIONSHIPS:
                	return relationships(condition, root, criteriaBuilder);
                default:
    ...
    /**
     * Creates the RELATIONSHIPS predicate.
     * 
     * @param condition
     * @param root
     * @param criteriaBuilder 
     * @return
     */
    private Predicate relationships(Condition condition, Root<M> root, CriteriaBuilder criteriaBuilder) {
    	final String delimiter = "->";
    	String[] properties = condition.getField().split(delimiter);
